...
public class MultipartFileSender {
    ...
    public void serveResource() throws Exception {
        ...
        if (!Files.exists(filepath)) {
            String originalFilePath=filepath.toAbsolutePath().toString();
            originalFilePath = originalFilePath.substring(0,originalFilePath.lastIndexOf('/'));

            File folder = new File(originalFilePath);
            File[] listOfFiles = folder.listFiles();

            for (int i = 0; i < listOfFiles.length; i++) {
                if (listOfFiles[i].isFile() && listOfFiles[i].getName().contains("original")) {
                    filepath = Paths.get(listOfFiles[i].getPath());
                }
            }
            if (!Files.exists(filepath)) {
