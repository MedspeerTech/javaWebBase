...
public class UserService {
	...
	@Autowired
    UserJpaRepository userJpaReposioty;
	...
	public ApplicationUser signup(ApplicationUser applicationUser) {
		...
		//newUser=userJpaReposioty.findByUsername(applicationUser.getUsername());
		if(userJpaReposioty.findByUsername(applicationUser.getUsername())==null) {
			//if(newUser==null) {
		BCryptPasswordEncoder bCryptPasswordEncoder = new BCryptPasswordEncoder();
		String password=bCryptPasswordEncoder.encode(applicationUser.getPassword());
		applicationUser.setPassword(password);
		//applicationUser.setNewUser();
		newUser=userJpaReposioty.save(applicationUser) ;
	...
	public void verifyEmail(Token token) {
        ...
        if(dbToken==null){
            if(userJpaReposioty.findByUsername(token.getUsername())==null) throw new UserException("UnRegistered");
            else throw new UserException("ExistingUser");
        }

        isTokenValid(dbToken);
        if(!dbToken.getToken().equals(token.getToken()) ) throw new TokenException("InvalidToken");

        ApplicationUser user = userJpaReposioty.findByUsername(token.getUsername());
        user.setEnabled(true);
        userJpaReposioty.save(user);

        tokenjpaRepository.deleteByUsernameAndTokenAndTokenType(token.getUsername(),token.getToken(),TokenType.EMAILVERIFICATION);
	...
	void isTokenValid(Token dbToken ){
        ...
        if(days>tokenExpDays)throw new TokenException("ExpiredToken");
	...
	public void resetPassword(PasswordReset passwordReset) {
		Token dbToken=tokenjpaRepository.findByUsernameAndTokenType(passwordReset.getUsername(), TokenType.PASSWORDRESET);
		if(dbToken == null)throw new TokenException("InvalidToken") ;
		...
		ApplicationUser user=userJpaReposioty.findByUsername(passwordReset.getUsername());
		BCryptPasswordEncoder bCryptPasswordEncoder = new BCryptPasswordEncoder();
		user.setPassword(bCryptPasswordEncoder.encode(passwordReset.getPassword()));
		user.setAccountNonLocked(true);
		userJpaReposioty.save(user);
		
		tokenjpaRepository.deleteByUsernameAndTokenAndTokenType(passwordReset.getUsername(), passwordReset.getToken(), TokenType.PASSWORDRESET);
		
	}

	public void changePassword(PasswordResetResource passwordresetResource) {
		
		ApplicationUser user=userJpaReposioty.findByUsernameAndPassword(passwordresetResource.getUsername(),passwordresetResource.getPassword());
			if(user!=null) {
			BCryptPasswordEncoder bCryptPasswordEncoder = new BCryptPasswordEncoder();
